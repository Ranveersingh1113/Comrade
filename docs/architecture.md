...
`GroupRoom` sends an authenticated request to `/agent/turn/stream`. The response is newline-delimited JSON because a normal `fetch` request can retain the Authorization header. Tool calls, tool output, and patch results become expandable `AgentActivity` cards. Durable `agent_runs` and `agent_steps` let the thread recover activity after a refresh.
